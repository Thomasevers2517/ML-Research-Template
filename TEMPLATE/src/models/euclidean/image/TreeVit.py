...
class TreeVIT(BaseModule):
    def __init__(self, input_shape: tuple, output_shape: tuple, num_layers: int, embedding_size: int, 
                 num_heads: int, patch_size: int, dropout: float = 0.4, T_Threshold=0, num_children=4):
        super(TreeVIT, self).__init__(input_shape=input_shape, output_shape=output_shape)
        
        self.patch_size = patch_size
        self.num_patches = (input_shape[1] // patch_size) * (input_shape[2] // patch_size)
        self.num_levels = math.log(self.num_patches, num_children) + 1
        assert self.num_patches.is_integer(), "Image size must be divisible by patch size"
        assert self.num_levels.is_integer(), "Number of patches must be a power of the number of children"
